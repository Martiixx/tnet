...
size_t difficultyBlocksCountByBlockVersion(uint8_t blockMajorVersion, uint32_t) const;
  ...
  Difficulty nextDifficultyLWMA(uint32_t blockIndex, std::vector<uint64_t> timestamps, std::vector<Difficulty> cumulativeDifficulties) const;
